...
def validate_command_structure(content: str, template_type: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not template_type:
        ...
        if not template_type:
            result["warnings"].append(
                "Could not auto-detect template type. Skipping structure validation."
            )
    ...
    if template_type == "rpiv":
        ...
        if (
            "decision table" not in content.lower()
            and "IF" not in content
            and "THEN" not in content
        ):
            result["passed"] = False
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: validate_command_structure.py <command_file.md> [template_type]",
            file=sys.stderr,
        )
        sys.exit(1)
