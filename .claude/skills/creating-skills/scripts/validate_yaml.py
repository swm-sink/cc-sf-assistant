    ...
    if "name" in frontmatter:
        ...
        if not re.match(r"^[a-z0-9]+(-[a-z0-9]+)*$", name):
            ...
            result["errors"].append(
                f'Invalid name format: "{name}" (must be kebab-case: lowercase, hyphens only)'
            )
    ...
    if "description" in frontmatter:
        ...
        if len(description) < 20:
            result["warnings"].append(
                f"Description too short: {len(description)} chars (recommend ≥50 chars for CSO)"
            )
        ...
        keyword_count = sum(
            1 for keyword in keyword_indicators if keyword.lower() in description.lower()
        )
        result["info"]["cso_keyword_count"] = keyword_count

        if keyword_count < 3:
            result["warnings"].append(
                f"Description has low CSO keyword count: {keyword_count} (recommend ≥3 for discoverability)"
            )
...
def format_human_readable(result: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append("YAML Frontmatter Validation")
    lines.append(f"{'='*60}")
