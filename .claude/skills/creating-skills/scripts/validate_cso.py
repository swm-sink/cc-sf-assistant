...
def validate_cso(content: str) -> Dict[str, Any]:
    """Validate CSO score."""
    result = {"validator": "validate_cso", "passed": True, "errors": [], "warnings": [], "info": {}}
    ...
    if cso_score < 0.7:
        ...
        if cso_data["trigger_score"] < 0.7:
            result["warnings"].append(
                "  • Add more trigger phrases (when, before, after, use when, need to)"
            )

        if cso_data["symptom_score"] < 0.7:
            result["warnings"].append(
                "  • Add symptom keywords (thinking, feeling, noticing, under pressure)"
            )

        if cso_data["agnostic_score"] < 0.7:
            result["warnings"].append(
                "  • Add technology-agnostic keywords (creating, implementing, workflow, process)"
            )

        if cso_data["example_score"] < 0.7:
            result["warnings"].append(
                "  • Add specific examples (Google Sheets, variance, budget, integration)"
            )

    # Check description length
    if len(description) < 50:
        result["warnings"].append(
            f"Description short: {len(description)} chars (recommend ≥50 for keyword richness)"
        )
...
def format_human_readable(result: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append("CSO (Claude Search Optimization) Validation")
    lines.append(f"{'='*60}")
    ...
    if "cso_score" in result["info"]:
        ...
        if cso_score >= target:
            lines.append("   ✅ Score meets target")
        else:
            lines.append("   ⚠️  Score below target")

        # Show pillar breakdown
        lines.append("\n🔍 Pillar Breakdown:")
        lines.append(
            f"   • Trigger Phrases: {result['info']['trigger_count']} → {result['info']['trigger_score']}"
        )
        lines.append(
            f"   • Symptom Keywords: {result['info']['symptom_count']} → {result['info']['symptom_score']}"
        )
        lines.append(
            f"   • Agnostic Keywords: {result['info']['agnostic_count']} → {result['info']['agnostic_score']}"
        )
        lines.append(
            f"   • Example Indicators: {result['info']['example_count']} → {result['info']['example_score']}"
        )
