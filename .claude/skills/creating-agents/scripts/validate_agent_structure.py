...
def detect_agent_type(content: str) -> Optional[str]:
    ...
    if (
        "Domain Specialist" in content
        or "domain specialist" in content
        or "Domain Expertise Areas" in content
    ):
        return "domain-specialist"

    # Check for researcher patterns
    if (
        "research analyst" in content.lower()
        or "Research Expertise Areas" in content
        or "Investigation Workflow" in content
    ):
        return "researcher"

    # Check for reviewer patterns
    if (
        "reviewer" in content.lower()
        or "Review Expertise Areas" in content
        or "Review Workflow" in content
        or "Verification Checklist" in content
    ):
        return "reviewer"
...
def validate_agent_structure(content: str) -> Dict[str, Any]:
    ...
    if len(sections) < expected_section_count:
        ...
        result["errors"].append(
            f"Too few sections: {len(sections)} (expected {expected_section_count})"
        )
    elif len(sections) > expected_section_count:
        result["warnings"].append(
            f"More sections than expected: {len(sections)} (expected {expected_section_count})"
        )
    ...
    if (
        "coordinate other agents" in content.lower()
        or "coordinates other agents" in content.lower()
    ):
        if (
            "does NOT coordinate other agents" not in content
            and "NEVER coordinate other agents" not in content
        ):
            result["errors"].append(
                "Agent appears to coordinate other agents (anti-pattern). Commands coordinate agents, not agents."
            )
